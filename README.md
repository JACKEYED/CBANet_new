...
## Train
1. Prepare a training image directory (all images under one folder).
2. Run:
```bash
python train.py --config config/OneEncoderPruner.json --train_data_dir PATH_TO_TRAIN_IMAGES --save_dir checkpoints/cbanet_train --epochs 30
```
3. Use the produced checkpoint (e.g. `checkpoints/cbanet_train/final_model.pth.tar`) in test:
```bash
